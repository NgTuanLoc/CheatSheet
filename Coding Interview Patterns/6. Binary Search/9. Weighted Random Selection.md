---
title: "9. Weighted Random Selection"
source: "https://bytebytego.com/courses/coding-patterns/binary-search/weighted-random-selection"
author:
published:
created: 2026-04-23
...
![9-weighted-random-selection-figure-1](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-1.svg)
...
![9-weighted-random-selection-figure-2](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-2.svg)
...
![9-weighted-random-selection-figure-3](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-3.svg)
...
![9-weighted-random-selection-figure-4](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-4.svg)
...
![9-weighted-random-selection-figure-5](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-5.svg)
...
![9-weighted-random-selection-figure-6](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-6.svg)
...
![9-weighted-random-selection-figure-7](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-7.svg)
...
![9-weighted-random-selection-figure-8](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-8.svg)
...
![9-weighted-random-selection-figure-9](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-9.svg)
...
![9-weighted-random-selection-figure-10](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-10.svg)

Image represents a simple illustration of prefix sums. The top line shows a list labeled 'weights' containing four integer values: 3, 1, 2, and 4, enclosed in square brackets, indicating an array or list data structure. Below this, another list labeled 'prefix\_sums' is shown, also enclosed in square brackets. This list contains the cumulative sums of the 'weights' list. Specifically, the first element of 'prefix\_sums' (3) is the same as the first element of 'weights'. The second element of 'prefix\_sums' (4) is the sum of the first two elements of 'weights' (3 + 1). The third element (6) is the sum of the first three elements of 'weights' (3 + 1 + 2), and the final element (10) is the sum of all elements in 'weights' (3 + 1 + 2 + 4). The arrangement visually demonstrates the direct relationship and calculation between the original 'weights' and their resulting 'prefix\_sums'.
...
![9-weighted-random-selection-figure-11](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-11.svg)
...
![9-weighted-random-selection-figure-13](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-13.svg)

Image represents a visual depiction of a two-pointer approach to finding a subarray with a target sum. Two orange rectangular boxes labeled 'left' and 'right' point downwards towards an array represented by \`prefix\_sums = \[3, 4, 6, 10\]\`. Beneath each element of the \`prefix\_sums\` array, its index (0, 1, 2, 3) is shown in gray. The number 6 in the \`prefix\_sums\` array is highlighted in light green. A separate line indicates \`target = 5\`. The arrows from 'left' and 'right' visually suggest the movement of pointers across the \`prefix\_sums\` array during the algorithm's execution, aiming to find a subarray whose sum equals the \`target\` value. The highlighted '6' likely represents a point where the algorithm might find a solution (or a relevant intermediate state), given that the difference between consecutive prefix sums could be used to determine the sum of subarrays.
...
![9-weighted-random-selection-figure-14](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-14.svg)

Image represents a visual depiction of a binary search algorithm's step within the context of prefix sums. The diagram shows four labeled boxes: 'left,' 'mid,' and 'right' representing index pointers, and a dashed box containing a conditional statement. 'left' and 'mid' point downwards to an array \[3, 4, 6, 10\] with indices 0, 1, 2, and 3 respectively displayed below each element. 'right' also points downwards to the same array. Below the array, 'target = 5' is specified. The dashed box displays the condition 'prefix\_sums\[mid\] < target,' which evaluates whether the prefix sum at the 'mid' index is less than the target value (5). If true, as indicated by the arrow, the 'left' pointer is updated to 'mid + 1,' effectively narrowing the search space in a binary search fashion. The light blue box around 'mid' and the blue arrow visually emphasize the current 'mid' index and its role in the conditional statement.

---

![9-weighted-random-selection-figure-15](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-15.svg)
...
![9-weighted-random-selection-figure-16](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-16.svg)

Image represents a visual depiction of a binary search algorithm step within a coding pattern context. The top shows three labeled boxes: 'left,' 'mid' (highlighted in cyan), and 'right.' Arrows point downwards from 'left' and 'right' to the array \[3, 4, 6, 10\], indicating index positions 0, 2, and 3 respectively. The 'mid' box points to the element '6' at index 2. Below the array, 'target = 5' is specified. A dashed box to the right shows a conditional statement: 'prefix\_sums\[mid\] ≥ target,' which evaluates whether the prefix sum at the midpoint (6 in this case) is greater than or equal to the target (5). A right-pointing arrow from this condition leads to the assignment 'right = mid,' indicating that if the condition is true, the right boundary of the search space is updated to the midpoint. The overall diagram illustrates a single iteration of a binary search, where the algorithm checks if the prefix sum at the midpoint meets or exceeds the target value to refine the search range.

---

![9-weighted-random-selection-figure-17](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-17.svg)
...
![9-weighted-random-selection-figure-18](../assets/6.%20Binary%20Search/9-weighted-random-selection/9-weighted-random-selection-figure-18.svg)
