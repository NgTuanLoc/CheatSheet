---
title: "1. Introduction to Sort and Search"
source: "https://bytebytego.com/courses/coding-patterns/sort-and-search/introduction-to-sort-and-search"
author:
published:
created: 2026-04-24
...
| Heapsort [\[4\]](https://en.wikipedia.org/wiki/Heapsort) | $O(n\log(n))$ | $O(n\log(n))$ | $O(n\log(n))$ | $O(1)$ |
| Counting sort [^1] | $O(n+k)$ | $O(n+k)$ | $O(n+k)$ | $O(n+k)$ |
| Bucket sort [^2] [\[5\]](https://en.wikipedia.org/wiki/Bucket_sort) | $O(n+k)$ | $O(n+k)$ | $O(n^2)$ | $O(n+k)$ |
| Radix sort [^3] [\[6\]](https://en.wikipedia.org/wiki/Radix_sort) | $O(d(n+k))$ | $O(d(n+k))$ | $O(d(n+k))$ | $O(n+k)$ |
...
![1-introduction-to-sort-and-search-figure-1](../assets/17.%20Sort%20And%20Search/1-introduction-to-sort-and-search/1-introduction-to-sort-and-search-figure-1.svg)
